...
def check_buy_order(code):
    ...
    for order in order_list:
        ...
        if order_cnt == result_cnt:
            mongo.update_item({"매수완료.주문번호": order_no}, {"$set": {"매수완료": check_result, "status": "buy_completed"}}, "stocklab_demo", "order")
...
def trading_scenario(code_list):
    for code in code_list:
        ...
        if buy_order_cnt == 0:
            # 종목을 보유하고 있지 않은 경우 매수하기
            order = ebest_demo.order_stock(code, "2", current_price, "2", "00")
            print("order_stock: ", order)
            order_doc = order[0]
            mongo.insert_item({"매수주문": order_doc, "code": code, "status": "buy_ordered"}, "stocklab_demo", "order")
...
if __name__ == "__main__":
    ...
    print("today: ", today)
    scheduler.add_job(func=run_process_trading_scenario, trigger="interval", minutes=5, id="demo", kwargs={"code_list": code_list})
    scheduler.start()
    while True:
